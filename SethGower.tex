%!TEX TS-program = xelatex
\documentclass[letterpaper, 11pt]{article}
    %--------DEPENDENCIES--------
        \usepackage{fontspec}
        \usepackage{latexsym}
        \usepackage[empty]{fullpage}
        \usepackage{titlesec}
        \usepackage{marvosym}
        \usepackage[usenames,dvipsnames,svgnames,table]{xcolor}
        \usepackage{verbatim}
        \usepackage{enumitem}
        \usepackage{hyperref}
        \usepackage{tabulary}
        \usepackage{tabularx}
        \usepackage{xspace}
        \usepackage{enumitem}
    %----------------------------

    \hypersetup{colorlinks=false,}

    %----SECTION-FORMATTING-----
        \titleformat{\section}{
        \vspace{-10pt}\raggedright\large
        }{}{0em}{}[\color{black}\titlerule \vspace{-5pt}]


    %---------------------------

    %--------FONT-SETUP---------
        \setmainfont[
            Path = fonts/,
            Extension =.ttf,
            UprightFont = *-Regular,
            ItalicFont = *-Italic,
            BoldFont = *-Bold,
            BoldItalicFont = *-BoldItalic,
        ]{Roboto}
        \newfontfamily\robotoLight[
            Path = fonts/,
            Extension =.ttf,
            UprightFont = *-Regular,
            ItalicFont = *-Italic,
        ]{RobotoLight}
        \newfontfamily\robotoMed[
            Path = fonts/,
            Extension =.ttf,
            UprightFont = *-Regular,
            ItalicFont = *-Italic,
        ]{RobotoMedium}
        \newfontfamily\robotoThin[
            Path = fonts/,
            Extension =.ttf,
            UprightFont = *-Regular,
            ItalicFont = *-Italic,
        ]{RobotoThin}
    %---------------------------

    %----------MARGINS----------
        \addtolength{\oddsidemargin}{-0.375in}
        \addtolength{\evensidemargin}{-0.375in}
        \addtolength{\textwidth}{1in}
        \addtolength{\topmargin}{-.75in}
        \addtolength{\textheight}{1.0in}
    %---------------------------

    %-----HEADER-SETUP---------
        %\pagestyle{fancy}
        %\fancyhf{} % clear all header and footer fields
        %\fancyfoot{}
        %\renewcommand{\headrulewidth}{0pt}
        %\renewcommand{\footrulewidth}{0pt}
    %---------------------------

    %--------NEW-COMMANDS-------
        %Creates tab command that tabs the text over the argument amount
            \newcommand{\tab}{\hspace*{.3cm}}
        %Accents Subsection Headings and List Item Titles
            \renewcommand{\accent}[1]{\textbf{\textcolor{Maroon}{#1}}}
        %Resume Subsection-Takes 5 Arguments [optional:link]{Title}{Location}{Description}{Duration}
            \newcommand{\resumeSubsection}[5][]{
                \item[]
                    \newcolumntype{R}{>{\raggedleft\arraybackslash}X}
                    \begin{tabularx}{1\textwidth}{p{4.5in}R}
                        \accent{#2}\hspace{.25cm}\footnotesize\robotoLight{\href{https://#1}{#1}} & {\mbox{{#3}}} \\
                        \small\robotoLight{#4} & \robotoLight{\mbox{#5}}
                    \end{tabularx}
            }
        %Project Items-Takes 4 arguments {Title}{Language}{gitlab}{Description}
            \newcommand{\project}[5]{
                    \item[]
                        \newcolumntype{R}{>{\raggedleft\arraybackslash}X}
                        \begin{tabularx}{1\textwidth}{p{2.1in}p{3.2in}R}
                            \accent{\mbox{#1}} & &  \mbox{#2}\\
                            \footnotesize\robotoThin{\href{https://#3}{#3}} & \small\robotoLight{#4} & \mbox{\robotoThin{#5}}\\
                        \end{tabularx}
            }
        %Resume Listed Item-Takes 2 arguments {Title}{Description}
            \newcommand{\resumeItem}[2]{\item[]\tab{\accent{#1}: \robotoLight{#2}}}
        %Commands for List Environment
            \newcommand{\resumeSubHeadingListStart}{\begin{itemize}[leftmargin=*]}
            \newcommand{\resumeSubHeadingListEnd}{\end{itemize}}


    %---------------------------

    \setlist{nosep}
    \begin{document}
        %------------HEADER---------

            \hspace{-0.65cm}\huge{\accent{Seth Gower}}
            \normalsize\newline
            \robotoLight{\href{mailto:seth@sethgower.com}{seth@sethgower.com}}\newline
            \robotoLight{\href{https://gitlab.com/sethgower}{gitlab.com/SethGower}}\newline
            \robotoLight{\href{http://sethgower.com}{sethgower.com}}\newline
            \robotoMed Looking for a co-op for Summer 2019
        %---------------------------

        %--------EDUCATION----------
            \section{Education}
                \resumeSubHeadingListStart
                    \resumeSubsection
                        {Rochester Institute of Technology}{Rochester, NY}
                        {B.S Computer Engineering, 5 year program\newline
						 Minor in Computer Security}{Expected Graduation of May 2022}
                \resumeSubHeadingListEnd
        %---------------------------
	
		%------SKILLS---------------
            \section{Skills}
                \resumeSubHeadingListStart
                    \resumeItem{Languages}{Java, Python, Arduino C, \LaTeX\xspace}
					\resumeItem{Tools}{Git, KiCad, Altera Quartus II, ModelSim, Jekyll, VHDL}
                    \resumeItem{Hardware}{Soldering, Prototyping on breadboard}
                    \resumeItem{Professional Skills}{Public Speaking, Spanish (Semi Conversational)}
                \resumeSubHeadingListEnd
        %---------------------------


        %-------PROJECTS------------
            \section{Projects}
                \resumeSubHeadingListStart
                    \resumeSubsection[gitlab.com/SethGower/SUDS]
                        {SUDS}{Hardware}
                        {24 Hour Hackathon Project. The Shower Use Detection System for my dorm floor, showing the occupancy status of the showers. Uses Limit switches on shower doors to close circuit and light up LED and send input to Raspbery Pi GPIO pin.}{October 2017}
					\vspace{.1cm}
                    \resumeSubsection[gitlab.com/SethGower/RGB-LED-API]
                        {GOLD}{Hardware, Arduino}
                        {An IoT project that enables the user to control an LED strip using the Google Assistant. Currently an Arduino receives commands via HTTP requests and changes the color of the strip.}{November 2017--Present}
                    \vspace{.1cm}
					\resumeSubsection[github.com/news-aggregator/news\_aggregator]
						{NewsMap}{Python, Flask, SQLAlchemy}
						{A News Aggregator created as a Hackathon project at DandyHacks 2018. Scrapes news from the internet using RSS and Twitter feeds. Places news on a map to show the dispersion of news.}{February 2018}
					%\resumeSubsection[gitlab.com/SethGower/personal-blog]
                    %    {My Personal Blog}{Jekyll}
                    %    {My personal blog, that I self host on my server and build with Jekyll, a static website generator based on Ruby. When I complete a project, or learn something new, I may post it here.}{November 2017}
                    \resumeSubHeadingListEnd
        %---------------------------
					
        %------WORK-EXP-------------
            \section{Work Experience}
                \resumeSubHeadingListStart
					\resumeSubsection{Audio Video Responder}{Rochester, NY}
					{\textit{Classroom Technology Support}\newline Provides technical support to professors and other patrons who use the classrooms and A/V systems provided by the University.}{January 2018--Present}
                    \resumeSubsection{Tech Camp Counselor}{Williamsburg, VA}
                    {\textit{iD Tech Camp Counselor}\newline Taught programming in Python, Java and basic Linux to early teenagers.}{June--July 2018} 
                \resumeSubHeadingListEnd
        %---------------------------

        %------LEADER-EXP-----------
           % \section{Leadership Experience}
           %     \resumeSubHeadingListStart
           %         \resumeSubsection{Robotics Team Captain}{Norfolk, VA}
           %             {\textit{Programmer} \newline Collaborated with other team members and programmers on the program for that year's competition. We finished 5th in the state.}{August 2016--February 2017}
           %         \resumeSubHeadingListEnd
        %---------------------------
		
		%-----Organizations---------
            \section{Organizations}
                \resumeSubHeadingListStart
                    \resumeSubsection[ehouse.rit.edu]
                        {Engineering House}{Rochester, NY}
                    	{Engineering House is a social floor with an academic interest. eHouse is a place where students gather to learn and socialize through project based learning and mentor-mentee relationships.}{October 2017--Present}
                    \resumeSubsection[codeRIT.org]
                        {codeRIT}{Rochester, NY}
                        {codeRIT is RIT's student organization that encourages the hacker culture at RIT by promoting other school's hackathons, and hosting BrickHack, RIT's premier hackathon, an MLH member event. I am an organizer for BrickHack, and am on the Travel, Logistics and Sponsorship committees.}{February 2018--Present}
                    %\resumeSubsection[csh.rit.edu]
                    %    {Computer Science House}{Rochester, NY}
                    %    {Computer Science House is an organization at RIT that encourages extracurricular learning, through individual projects, collaboration, and hands on learning.}{August 2017--April 2018}
					\resumeSubHeadingListEnd

        %---------------------------


    \end{document}

